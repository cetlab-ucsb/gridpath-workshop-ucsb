...
# Grab data from databases for plotting state of charge
def _load_stor(scen_labels_, path, gp_model):

    # Load stage of charge table and process data from database
    def __load_soc_from_csv(df_, scenario, zones_):
        ...
        df_ = df_.loc[df_['load_zone'].isin(zones_)].reset_index(drop = True)
    ...
    for scenario in scenarios_:
        ...
        if gp_model == 'production':
            dispatch_ = []
            for folder in next(os.walk(dir_name))[1]:
                dispatch_.append(pd.read_csv(dir_name + f'/{folder}/results/project_timepoint.csv', low_memory = False))
            dispatch_ = pd.concat(dispatch_, axis = 0).reset_index(drop = True)
        if gp_model == 'capex':
            dir_name  = r'{}/{}'.format(path, scen)
            dispatch_ = pd.read_csv(dir_name + f'/results/project_timepoint.csv', low_memory = False)
        dfs_ += [__load_soc_from_csv(dispatch_, scenario, zones_)]

    return pd.concat(dfs_, axis = 0).reset_index(drop = True)
    
    
def _load_resource_adequacy_demand(scen_labels_, path, gp_model):
    
    # Load energy dispatch table and process data from database
    def __load_demand_from_csv(df_, scenario, zones_):
        ...
        df_p_ = df_p_['load_zone'].loc[df_p_['load_zone'].isin(zones_)].reset_index(drop = True)
    ...
    for scenario in scenarios_:
        ...
        dfs_ += [__load_demand_from_csv(demand_, scenario, zones_)]
...
__all__ = ['_load_capacity',
           '_load_stor',
           '_load_resource_adequacy_demand',
