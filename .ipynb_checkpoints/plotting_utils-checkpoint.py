...
def _plot_soc(soc_, scen_labels_, tech_labels_,
    ...
    for k in range(scen_labels_.shape[0]):     

        row_  = scen_labels_.iloc[k]
        scen  = row_['scenario']
        zone  = row_['zone']
        label = row_['label']
        ls    = row_['linestyle']
    
        idx_   = (soc_['scenario'] == scen)
        ...
        for tech in techs_:
            ...
            if (y_ > 0.).sum() > 0.:
                y_ = 100.*y_/y_.max()
                ax.plot(x_, y_, label = '{} ({})'.format(label, tech.replace("_", " ")), 
                                lw = 2.5, c = color, ls = ls)
                
            ax.set_ylabel(units_label, fontsize = 16)
